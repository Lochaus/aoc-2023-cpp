...
int rgb_count(std::string input){
    ...
    std::map<std::string, int> maxColourFound { {"red", 0}, {"green", 0}, {"blue", 0}};
    for (int i = 0; i < subGame.size(); i++){
        ...
        if (colourCounter["red"] > maxColourFound["red"]){
          maxColourFound["red"] = colourCounter["red"];
        }
        if (colourCounter["green"] > maxColourFound["green"]){
          maxColourFound["green"] = colourCounter["green"];
        }
        if (colourCounter["blue"] > maxColourFound["blue"]){
          maxColourFound["blue"] = colourCounter["blue"];
        }
    }
    return (maxColourFound["red"] * maxColourFound["green"] * maxColourFound["blue"]);
}
...
int main() {
    ...
    for (int i = 0; i < inputLines.size(); i++){
        solution += rgb_count(inputLines[i]);
    }
